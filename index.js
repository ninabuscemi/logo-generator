...
async function runLogoGenerator() {
  ...
  function generateSVG({ text, textColor, shape, shapeColor }) {
    ...
    switch (shape) {
      case 'circle':
        svgShape = new Circle(shapeSize / 2, shapeColor).draw();
        break;
      case 'triangle':
        const triangleWidth = 200;
        svgShape = new Triangle(shapeColor).draw(triangleWidth, 100, 100);
        break;
      case 'square':
        svgShape = new Square(shapeSize, shapeColor).draw();
        break;
      default:
